...
module main =
  ...
  type deletion_itemsReader(reader: System.Data.Common.DbDataReader, getOrdinal) =
    member __.deletion_id =
      RequiredColumn(reader, getOrdinal, reader.GetInt64, "deletion_id")

    member __.item_name = RequiredColumn(reader, getOrdinal, reader.GetString, "item_name")

    member __.item_serial =
      OptionalColumn(reader, getOrdinal, reader.GetString, "item_serial")
    ...
    member __.is_deletion =
      RequiredColumn(reader, getOrdinal, reader.GetBoolean, "is_deletion")

    member __.is_hidden =
      RequiredColumn(reader, getOrdinal, reader.GetBoolean, "is_hidden")

    member __.to_location =
      OptionalColumn(reader, getOrdinal, reader.GetString, "to_location")
  ...
  type employerReader(reader: System.Data.Common.DbDataReader, getOrdinal) =
    ...
    member __.first_name =
      RequiredColumn(reader, getOrdinal, reader.GetString, "first_name")

    member __.last_name = RequiredColumn(reader, getOrdinal, reader.GetString, "last_name")

    member __.is_approved =
      RequiredColumn(reader, getOrdinal, reader.GetBoolean, "is_approved")
  ...
  [<CLIMutable>]
  type message = { chat_id: int64; message_json: string }

  type messageReader(reader: System.Data.Common.DbDataReader, getOrdinal) =
    member __.chat_id = RequiredColumn(reader, getOrdinal, reader.GetInt64, "chat_id")

    member __.message_json =
      RequiredColumn(reader, getOrdinal, reader.GetString, "message_json")
  ...
  type officeReader(reader: System.Data.Common.DbDataReader, getOrdinal) =
    ...
    member __.office_name =
      RequiredColumn(reader, getOrdinal, reader.GetString, "office_name")

    member __.is_hidden =
      RequiredColumn(reader, getOrdinal, reader.GetBoolean, "is_hidden")

    member __.manager_id =
      RequiredColumn(reader, getOrdinal, reader.GetInt64, "manager_id")
...
type HydraReader(reader: System.Data.Common.DbDataReader) =
  ...
  let lazymainchat_id_table =
    lazy (main.chat_id_tableReader (reader, buildGetOrdinal 1))

  let lazymaindeletion_items =
    lazy (main.deletion_itemsReader (reader, buildGetOrdinal 11))
  ...
  member __.AccFieldCount
    with private get () = accFieldCount
    and private set (value) = accFieldCount <- value
  ...
  static member private GetPrimitiveReader
    ...
    if t = typedefof<int16> then
      Some(wrap reader.GetInt16)
    else if t = typedefof<int> then
      Some(wrap reader.GetInt32)
    else if t = typedefof<double> then
      Some(wrap reader.GetDouble)
    else if t = typedefof<System.Single> then
      Some(wrap reader.GetDouble)
    else if t = typedefof<decimal> then
      Some(wrap reader.GetDecimal)
    else if t = typedefof<bool> then
      Some(wrap reader.GetBoolean)
    else if t = typedefof<byte> then
      Some(wrap reader.GetByte)
    else if t = typedefof<int64> then
      Some(wrap reader.GetInt64)
    else if t = typedefof<byte []> then
      Some(wrap reader.GetValue)
    else if t = typedefof<string> then
      Some(wrap reader.GetString)
    else if t = typedefof<System.DateTime> then
      Some(wrap reader.GetDateTime)
    else if t = typedefof<System.DateOnly> then
      Some(wrap reader.GetFieldValue)
    else if t = typedefof<System.TimeOnly> then
      Some(wrap reader.GetFieldValue)
    else if t = typedefof<System.Guid> then
      Some(wrap reader.GetGuid)
    else
      None
  ...
  static member Read(reader: System.Data.Common.DbDataReader) =
    ...
    let buildEntityReadFn (t: System.Type) =
      let t, isOpt =
        if t.IsGenericType
           && t.GetGenericTypeDefinition() = typedefof<Option<_>> then
          t.GenericTypeArguments.[0], true
        else
          t, false
    ...
    if FSharp.Reflection.FSharpType.IsTuple(t) then
      let readEntityFns =
        FSharp.Reflection.FSharpType.GetTupleElements(t)
        |> Array.map buildEntityReadFn
