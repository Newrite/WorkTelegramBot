...
//==============================================
// Helpers for Result type, AsyncResult type and Option type
//==============================================

/// Functions for Option type (functor and monad).
[<RequireQualifiedAccess>] // RequireQualifiedAccess forces the `Option.xxx` prefix to be used
module Option =
  ...
  type OptionBuilder() =
    ...
    member self.For(sequence: seq<_>, body) =
      self.Using(
        ...
        fun enum -> self.While(enum.MoveNext, self.Delay(fun () -> body enum.Current))
      )
...
module AsyncResultComputationExpression =
  ...
  type AsyncResultBuilder() =
    ...
    member __.Delay(generator: unit -> AsyncResult<'T, 'TError>) : AsyncResult<'T, 'TError> =
    ...
    member self.While
      (
        guard: unit -> bool,
        computation: AsyncResult<unit, 'TError>
    ...
    member self.For
      (
        sequence: #seq<'T>,
        binder: 'T -> AsyncResult<unit, 'TError>
      ) : AsyncResult<unit, 'TError> =
      self.Using(
        sequence.GetEnumerator(),
