...
module Update =
  ...
  let update (history: System.Collections.Generic.Stack<_>) message model callInitilizationModelFunction =
    match message with
    | UpdateMessage.Back | UpdateMessage.Cancel | UpdateMessage.NothingChange -> ()
    | _ -> history.Push(model)
    match message with
    | UpdateMessage.AuthManagerChange  newAuth -> newAuth |> AuthProcess.Model.Manager  |> CoreModel.Auth
    | UpdateMessage.AuthEmployerChange newAuth -> newAuth |> AuthProcess.Model.Employer |> CoreModel.Auth
    | UpdateMessage.DeletionProcessChange (state, newProces) ->
      EmployerProcess.Model.Deletion newProces |> state.UpdateModel |> CoreModel.Employer
    | UpdateMessage.StartEditRecordedItems state ->
      { state with Model = EmployerProcess.Model.EditRecordedDeletions } |> CoreModel.Employer
    | UpdateMessage.StartAuthEmployers (state, office) ->
      { state with Model = ManagerProcess.Model.AuthEmployers office } |> CoreModel.Manager
    | UpdateMessage.StartDeAuthEmployers (state, office) ->
      { state with Model = ManagerProcess.Model.DeAuthEmployers office } |> CoreModel.Manager
    ...
    | UpdateMessage.ManagerChooseOffice (state, office) ->
      { state with Model = ManagerProcess.Model.InOffice office } |> CoreModel.Manager
    | UpdateMessage.ManagerMakeOfficeChange (state, newChange) ->
      { state with Model = ManagerProcess.Model.MakeOffice newChange } |> CoreModel.Manager
    | UpdateMessage.FinishManagerAuth (manager, env) ->
      ...
      | Error err ->
        let text =
          $"Добавление позиции в базу данных не удалось: {err}"
    ...
    | UpdateMessage.NothingChange ->
      model
    | UpdateMessage.Back -> if history.Count > 0 then history.Pop() else model
