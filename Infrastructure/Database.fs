...
module Database =
  ...
  let private sharedQueryContext env =
    ...
    Shared <| new QueryContext(env.DBConn, Compilers.SqliteCompiler())
    ...
    task {
      try 
        ...
        if inserted > 0 then
          ...
          return DatabaseError.ChatIdAlreadyExistInDatabase chatId |> Error
  ...
  let insertManagerAsync env (managerToAdd: RecordedManager) =
    ...
    task {
      ...
      try
        ...
        if inserted > 0 then
          env.Log.Debug 
            $"Success insert new manager, chat id = {managerToAdd.ChatId}"
          CacheCommand.AddManager managerToAdd |> env.CacheActor.Post
      ...
      | Some o ->
        ...
        try
          ...
          if inserted > 0 then
            ...
            CacheCommand.AddEmployer employerToAdd |> env.CacheActor.Post
        ...
        with exn ->
          ...
          return Error exn.Message
      | None ->
        env.Log.Error $"Failed insert employer with chat id = {employerToAdd.ChatId}
        ...
        return
         Some
          { FirstName = %e.first_name
            ...
            ChatId     = %e.chat_id }
      | None   ->
        env.Log.Warning $"Failed select employer by chat id = {chatId}
    ...
    task {
      ...
      try
        ...
        if inserted > 0 then
          ...
          env.Log.Debug
            $"Success insert new office with manager chat id = {chatId}, update Cache"
          office |> CacheCommand.AddOffice |> env.CacheActor.Post
 ...
 /// Insert office in database and update office list in cache
  ...
  let internal initializeCacheAsync env =
    task {
      ...
      let employersCache =
        ...
        |> Seq.map (fun e ->
          let office =
            let o = offices |> Seq.find (fun o -> o.office_id = e.office_id)
            officesCache
      ...
      return
        ...
        |> List.map (fun di ->
          ...
          { Item  = item
            ...
            Location = location
            Employer = employer }, di.deletion_id)
  ...
  let updateIsApprovedEmployerAsync env isApproved (employer: RecordedEmployer) =
    task {
      ...
      env.Log.Info 
        $"Try update approve to {isApproved} for employer with chat id {chatId}"
      ...
      try
        ...
        if updated > 0 then
          env.Log.Debug 
            $"Success set {isApproved} in approved field employer in chat id {chatId}"
          return employer |> Ok
        else
          env.Log.Warning 
            $"Approved field of employer is not updated to {isApproved}"
          return employer |> DatabaseError.CantUpdateEmployerApproved |> Error
      with :? Microsoft.Data.Sqlite.SqliteException as qe ->
          env.Log.Error
  ...
  let setIsHiddenTrueForItemAsync env itemId =
    task {
      ...
      env.Log.Info 
        $"Try set is_hidden item to true for item with id {itemId}"
      ...
      try
        ...
        if updated > 0 then
          env.Log.Debug 
            $"Success set is_hidden item to true for item with id {itemId}"
          return itemId |> Ok
        else
          env.Log.Warning 
            $"Failed set is_hidden item to true for item with id {itemId}"
          return itemId |> DatabaseError.CantDeleteRecordedItem |> Error
      with :? Microsoft.Data.Sqlite.SqliteException as qe ->
          env.Log.Error
    ...
    task {
      ...
      try
        ...
        if deleted > 0 then
          ...
          CacheCommand.DeleteOffice office |> env.CacheActor.Post
          return office |> Ok
        else
    ...
    task {
      ...
      try
        ...
        let! selected =
          selectTask HydraReader.Read queryContext {
            for di in deletionItems do
            join e in employers on (subqueryOne queryOfficeId = e.office_id)
