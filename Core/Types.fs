...
[<AutoOpen>]
module UMX =
  [<Measure>] type private itemname
  [<Measure>] type private serialnumber
  [<Measure>] type private macaddress
  [<Measure>] type private lastname
  [<Measure>] type private firstname
  [<Measure>] type private officename
  [<Measure>] type private location
...
module rec Types =
  ...
  [<RequireQualifiedAccess>]
  type MacAddressError =
   | CantValidateMacAddress
  ...
  module MacAddress =
    ...
    let validate (input: string): Result<MacAddress, MacAddressError> =
      ...
      | true ->
        ...
        %macAddress |> Ok
      | false ->
        MacAddressError.CantValidateMacAddress |> Error
  ...
  module OfficeName =
    ...
    let equals officeOne officeTwo =
      ...
      let officeString (officeName: OfficeName) =
        ...
        str.ToLower().Replace(" ", "").Replace(":", "").Replace(";", "").Replace("-", "")
  ...
  [<Struct>]
  type PositiveInt = private { Value: uint } with
  ...
  module Item =
    ...
    let createWithSerial     name serial =
      ItemWithSerial.create name serial                |> Item.ItemWithSerial

    let createWithMacAddress name serial macaddress =
      ItemWithMacAddress.create name serial macaddress |> Item.ItemWithMacAddress

    let createWithOnlyName   name =
      ItemWithOnlyName.create name                     |> Item.ItemWithOnlyName
  ...
  type RecordedDeletionItem =
    ...
    override self.ToString() =
      let macText =
        if self.Item.MacAddress.IsSome then %self.Item.MacAddress.Value else "Нет"
      let serialText =
        if self.Item.Serial.IsSome     then %self.Item.Serial.Value     else "Нет"
      let locationText =
        if self.Location.IsSome        then %self.Location.Value        else "Не указано"
